...
exports.getSingleOrder = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!order) {
    return next(new ErrorHandler('Order not found bro.', 404));
  }
  res.status(200).json({
    success: true,
    order,
...
exports.getOrders = catchAsyncErrors(async (req, res, next) => {
  ...
  const apiFeatures = new APIFeatures(Orders.find(), req.query)
    .search()
    .filter()
...
exports.updateOrder = catchAsyncErrors(async (req, res, next) => {
  ...
  if (order.orderStatus === 'Delivered') {
    return next(new ErrorHandler('You have already delivered this oder.', 404));
  }
